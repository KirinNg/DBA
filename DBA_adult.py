...
def load_G(path, batch_size=32):
    df = pd.read_csv(path, header=None, encoding='utf-8', index_col=None, comment='|')
    ...
    df.loc[df['工资'] == ' <=50K.', '工资'] = 0
    df.loc[df['工资'] == ' >50K.', '工资'] = 1
    df.loc[df['人种'] != ' White', '人种'] = 0
    df.loc[df['人种'] == ' White', '人种'] = 1
    df.loc[df['性别'] == ' Male', '性别'] = 1
    df.loc[df['性别'] == ' Female', '性别'] = 0
    df['年龄'] = df['年龄'].astype(int)
    df['每周工作时间'] = df['每周工作时间'].astype(int)
    df['工资'] = df['工资'].astype(int)
    df['人种'] = df['人种'].astype(int)
    df['性别'] = df['性别'].astype(int)
    df_categorical = [i for i in df.columns if df[i].dtype.name == 'object']
    df_numerical = [i for i in df.columns if df[i].dtype.name != 'object']
    df_tree = pd.concat([df[df_numerical], pd.get_dummies(df[df_categorical])], axis=1)
    df_x = np.float32(np.asarray(df_tree.drop(['工资'], axis=1)))
    df_y = np.float32(np.asarray(df_tree['工资']))
    df_s = np.float32(np.asarray(df['人种']))
    generator = tf.data.Dataset.from_tensor_slices((df_x, df_y, df_s)).batch(batch_size)
    return generator


def load_adult(BATCH_SIZE):
    train_ds = load_G("/home/kirin/PycharmProjects/DBA/Adult_data/adult.data", BATCH_SIZE)
    test_ds = load_G("/home/kirin/PycharmProjects/DBA/Adult_data/adult.test", BATCH_SIZE)
    return train_ds, test_ds, test_ds


train_data, _, test_data = load_adult(128)
...
for epoch in range(EPOCHS):
    ...
    for Pick in tqdm.tqdm(test_data):
        img = Pick[0]
        img = tf.concat((img, tf.zeros([img.shape[0],  3])), axis=-1)
...
print(ans_line)
# odd:0.03630807626481036 EACC:0.8166535766543161